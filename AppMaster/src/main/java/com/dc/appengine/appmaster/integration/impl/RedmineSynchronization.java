...
public class RedmineSynchronization implements IIntegrationSynchronization {
	private static final Logger log = LoggerFactory.getLogger(RedmineSynchronization.class);

	private transient static RedmineSynchronization instance;

	private RedmineSynchronization() {
	}

	public static RedmineSynchronization getInstance() {
		if (instance == null) {
			synchronized (RedmineSynchronization.class) {
				if (instance == null) {
					instance = new RedmineSynchronization();
				}
			}
		}
		return instance;
