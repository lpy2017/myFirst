...
abstract class OpenSshCertificateDecoder extends CertificateDecoder {
    ...
    public KeyPair createKeyPair(PEMStructure pemStructure, String password) throws IOException {
        ...
        if (!"openssh-key-v1".equals(new String(header, "utf-8").trim())) {
        ...
        if ("bcrypt".equals(kdf)) {
            ...
            privateKeys = decryptData(privateKeys, generateKayAndIvPbkdf2(password.getBytes("utf-8"), salt, rounds, sshCipher.getKeyLength(), sshCipher.getBlockSize()), sshCipher);
