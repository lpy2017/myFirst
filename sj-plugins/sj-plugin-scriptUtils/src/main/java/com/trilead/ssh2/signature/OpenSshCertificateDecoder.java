...
abstract class OpenSshCertificateDecoder extends CertificateDecoder {
    ...
    public KeyPair createKeyPair(PEMStructure pemStructure, String password) throws IOException {
        ...
        if (!"openssh-key-v1".equals(new String(header, Charset.forName("UTF-8")).trim())) {
        ...
        if ("bcrypt".equals(kdf)) {
            ...
            privateKeys = decryptData(privateKeys, generateKayAndIvPbkdf2(password.getBytes(Charset.forName("UTF-8")), salt, rounds, sshCipher.getKeyLength(), sshCipher.getBlockSize()), sshCipher);
