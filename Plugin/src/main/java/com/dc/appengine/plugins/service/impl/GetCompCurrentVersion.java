...
public class GetCompCurrentVersion extends AbstractPlugin {
	private static Logger log = LoggerFactory.getLogger(GetCompCurrentVersion.class);
	...
	public String doInvoke() {
		...
		Runnable runnable= new Runnable() {
			...
			public void run() {
				...
				resultMap.put(Constants.Plugin.MESSAGE, "获取当前版本"+Constants.Plugin.CurrentVersion_Key+"信息，成功！");
	...
	public static void main(String[] args) {
		    ...
		    Map<String,Object> unzip = new HashMap();
		    unzip.put("filePath", "c:/test/file.zip");
		    target.put("unzip", unzip);
		    current.put("unzip", unzip);
		    message.put("current", current);
		    message.put("target", target);
		    params.put("message", message);
		    String param= JSON.toJSONString(params, SerializerFeature.DisableCircularReferenceDetect);
		    System.out.println("转换前====="+param);
		    Map<String,Object> paramMap= JSON.parseObject(param, new TypeReference<Map<String, Object>>() {
			});
			Map<String,Object> messageMap = (Map<String, Object>)paramMap.get("message");
			if((Map<String, Object>) messageMap.get(Constants.Plugin.CurrentVersion_Key) !=null){
				messageMap.putAll((Map<String, Object>) messageMap.get(Constants.Plugin.CurrentVersion_Key));
				paramMap.put(Constants.Plugin.MESSAGE, messageMap);
			}
			System.out.println("转换后====="+JSON.toJSONString(paramMap,SerializerFeature.DisableCircularReferenceDetect));
		}
}
