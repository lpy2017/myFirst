...
public class InstallNode extends AbstractPlugin {
	@SuppressWarnings("unused")
	private static Logger log = LoggerFactory.getLogger(InstallNode.class);
	//CMD命令方法的key
...
//		DoAgentThreadPool.getInstance().execute(runnable);
	...
	public String doAgent() throws IOException, InterruptedException{
		...
		if(JudgeUtil.isEmpty(this.userName)||JudgeUtil.isEmpty(this.password)){
			resultCmd.put(Constants.Plugin.RESULT,false);
			resultCmd.put(Constants.Plugin.MESSAGE,"用户或密码为空！");
			return JSON.toJSONString(resultCmd);
		}
	...
	public void setFields() {
		...
		if(!JudgeUtil.isEmpty(this.pluginInput.get(USERNAME))){
			...
			this.userName =this.messageMap.get(USERNAME).toString();
		}
		if(!JudgeUtil.isEmpty(this.pluginInput.get(PASSWORD))){
			this.password = this.pluginInput.get(PASSWORD).toString();
		}else{
			//message根的password需要解密
			this.password=this.messageMap.get(PASSWORD).toString();
//			this.password = decryptRoot(PASSWORD);
		}
		...
		if(!JudgeUtil.isEmpty(this.pluginInput.get(IP))){
			...
			this.ip =this.messageMap.get(Constants.Plugin.NODEIP).toString();
		}
